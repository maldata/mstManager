...
class Episode(models.modelBase.Base):
    ...
    episodeNumber = sqlalchemy.Column(sqlalchemy.Integer, unique=True)
    episodeName = sqlalchemy.Column(sqlalchemy.String, nullable=False)

    season_id = sqlalchemy.Column(sqlalchemy.Integer, sqlalchemy.ForeignKey('seasons.id'))

    season = sqlalchemy.orm.relationship("Season", back_populates="episodes")
